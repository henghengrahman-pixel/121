...
app.get('/api/version', (req, res) => res.json({ ok: true, version: '1.0.2', service: 'deposit-railway-sync-server', wibNow: nowWibText() }));

app.get('/api/test-telegram', requireApiKey, async (req, res) => {
  const text = '✅ Test Telegram berhasil\nServer Railway aktif.\nWaktu WIB: ' + nowWibText();
  const result = await sendTelegram(text);
  res.status(result.ok ? 200 : 500).json(result);
});

app.post('/api/report-deposit', requireApiKey, async (req, res, next) => {
  req.body = {
    ...req.body,
    deposits: [{
      id: req.body.depositId || req.body.id || req.body.key,
      key: req.body.depositId || req.body.id || req.body.key,
      user: req.body.user,
      date: req.body.trxDateWib || req.body.date,
      amount: req.body.amount,
      balance: req.body.balance,
      bankAsal: req.body.bankAsal,
      info: req.body.info,
      ageText: req.body.ageMinutes ? String(req.body.ageMinutes) + ' menit' : req.body.ageText,
      deviceName: req.body.deviceName || req.body.source || 'chrome-extension'
    }]
  };
  return reportDepositsHandler(req, res);
});


async function reportDepositsHandler(req, res) {
...
app.post('/api/report-deposits', requireApiKey, reportDepositsHandler);
